...
import game.duofan.kenshi.action.JianQiaoAction;

public class JianQiao extends CustomCard implements ICardContainer {
    public static final String ID = IDManager.getInstance().getID(JianQiao.class);
    private static final CardStrings CARD_STRINGS = CardCrawlGame.languagePack.getCardStrings(ID); // 从游戏系统读取本地化资源
    private static final String NAME = CARD_STRINGS.NAME; // 读取本地化的名字
    private static final String IMG_PATH = "img/cards/jianQiao_skill.png";
    ...
    public void triggerOnEndOfTurnForPlayingCard() {
        ...
        if (rightCard.type != CardType.ATTACK) {
            return;
        }
    ...
    private static final float[] CARD_OFFSETS_X = new float[]{0}; // 子卡牌横向偏移
    private static final float[] CARD_OFFSETS_Y = new float[]{60};
    private static final float CARD_SCALE = 0.5f;     // 子卡牌缩放比例
    ...
    public void update() {
        super.update();
        target = g != null && g.size() == capacity ? CardTarget.ENEMY : CardTarget.NONE;
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new JianQiaoAction(g, m));
    }
