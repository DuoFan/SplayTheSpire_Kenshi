...
public class ZhanYanLuoPatch {
    @SpireInsertPatch(rloc = 0)
    public static void Patch(AbstractMonster __instance) {
        if (__instance.hasPower(ZhanYanLuo.POWER_ID)) {
            AbstractPlayer p = AbstractDungeon.player;
            if (p == null) {
                return;
            }

            if (p.hasRelic(YiQi.ID)) {
                return;
            }
            ...
            EventManager.getInstance().notifyEvent(EventKey.ON_GAIN_YI_QI, __instance, null);
        }
    }
}
